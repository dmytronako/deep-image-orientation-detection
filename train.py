...
def train(args):
    ...
    try:
        if config.USE_CACHE:
            ...
            base_dataset = ImageOrientationDatasetFromCache(cache_dir=config.CACHE_DIR, transform=None)
            logging.info(f"Successfully loaded dataset from CACHE ({len(base_dataset)} images).")
        else:
            logging.info("Using ON-THE-FLY image processing (caching is disabled).")
            base_dataset = ImageOrientationDataset(upright_dir=args.data_dir, transform=None)
            logging.info(f"Successfully loaded dataset for on-the-fly processing.")
            logging.info(f"Dataset found {len(base_dataset.image_files)} original image files.")
    ...
    train_loader = DataLoader(train_subset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=pin_memory_enabled, persistent_workers=True)
    val_loader = DataLoader(val_subset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=pin_memory_enabled, persistent_workers=True)
